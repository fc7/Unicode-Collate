...
# the following is modelled after List::Util
use vars qw(@ISA);
eval {
  ...
  eval {
    ...
    bootstrap Unicode::Collate $XS_VERSION;
  };
} unless $ENV{UNICODE_COLLATE_PP};

# load the pure Perl subs as fallback in case the XS version is not available:
eval << 'EOP' if not defined &unpack_U;
...
sub getmap {
    ...
    if (defined $self->{undefName}) {
        require Unicode::UCD;
        my $ci = Unicode::UCD::charinfo($key) ;
        return
            if $ci->{name} =~ /$self->{undefName}/;
    }
...
sub splitEnt
{
    ...
    for (my $i = 0; $i < @src; $i++) {
        $src[$i] = undef
            ...
            $self->getmap( $src[$i] ) && @{ $self->getmap( $src[$i] ) } == 0);
    }
    ...
    for (my $i = 0; $i < @src; $i++) {
        ...
        # Blocked combining character should not be contracted.
            ...
            # $self->{normCode} is false in the case of "prenormalized".
            {
                ...
                for (my $p = $i + 1; $p < @src; $p++) {
                    ...
                    if ($preCC != $curCC && $self->getmap($jcps.$tail)) {
                        $jcps .= $tail;
                        $src[$p] = undef;
                    } else {
                        $preCC = $curCC;
        ...
        if ($self->getmap($jcps) && @{ $self->getmap($jcps) } == 0) {
            if ($wLen && @buf) {
                $buf[-1][2] = $i + 1;
            }
            next;
...
sub getWt
{
    ...
    if (Hangul_SIni <= $u && $u <= Hangul_SFin) {
        ...
        if ($hang) {
            ...
            @hangulCE = map({
                $self->getmap($_) ? @{ $self->getmap($_) } : $der->($_);
            } @decH);
        }
...
sub index {
    ...
    @$strE or return wantarray ? () : NOMATCHPOS;
